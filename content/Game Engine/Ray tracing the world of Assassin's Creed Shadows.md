...
### 3.3 第三步：屏幕空间光线步进（Screen Space Ray Marching）
...
### 3.4 第四步：光线追踪回退（Ray Tracing Fallback）
...
## 四、间接光照与探针体积（Indirect Illumination & Probe Volume）
...
## 五、光线追踪场景定义（Ray Tracing Scene Definition）
...
### 5.3 三角形数据预烘焙（Baked Triangle Data）
...
## 六、材质系统（Material System）

### 6.1 统一材质描述（Unified Material Description）
...
## 八、探针体积的数据结构与管理（Probe Volume Data）
...
## 九、探针更新优化策略（Probe Update Optimization）
...
## 十、探针更新详细流程（Probe Update Pipeline Details）

### 10.1 元数据预处理（Metadata Pre-Update）
...
### 10.2 GBuffer 更新（Full GBuffer Update）
...
### 10.3 太阳阴影计算（Sun Shadow in GBuffer）
...
## 十一、光照快速变化的探针重缩放（Probe Rescaling for Fast Sun Movement）

### 11.1 问题描述

- 当太阳快速移动时（如快速时间过渡），每帧只更新少量探针会导致 **明显的逐帧光照不一致**，能清楚看到探针被分批更新的视觉瑕疵

### 11.2 解决方案：全局光照重缩放（Global Lighting Rescaling）
...
## 十二、探针光照计算的双 Pass 策略（Split Lighting Computation）
...
## 十二（续）、探针光照输出与缩放因子（Probe Lighting Output & Rescaling）
...
## 十三、探针卷积流程（Probe Convolution Pipeline）

### 13.1 可见性卷积：方差阴影贴图（Variance Shadow Map）
...
## 十五、逐像素漫反射光照 Pass（Per-Pixel Diffuse Illumination Pass）
...
### 15.5 输出编码：球谐函数（Spherical Harmonics）
...
## 十七、间接镜面反射光照（Indirect Specular Illumination）
...
## 十八、镜面反射的降噪挑战（Specular Denoising）
...
### 18.2 BRDF 解调（BRDF Demodulation）
...
### 18.4 亮度空间压缩降噪（Luminance Space Compression）
...
## 十九、Alpha Test 材质的性能挑战（Alpha Tested Materials）
...
### 19.3 不透明度近似方案（Shipped Solution for Diffuse）
...
## 二十一、光照泄漏问题与解决方案（Light Leaks）

光照泄漏是实时全局光照方案中的 **已知经典问题**。本作将泄漏来源分为 **直接光照泄漏** 和 **间接光照泄漏** 两大类。

---

### 21.1 直接光照泄漏（Direct Illumination Leaks）
...
### 21.2 间接光照泄漏（Indirect Illumination Leaks）
...
### 21.3 探针查询的偏移策略（Query Position Offsets）
...
### 21.5 室内体积分类（Indoor Volume Classification）
...
## 二十三、降噪的特殊挑战与解决方案（Denoising Challenges）
