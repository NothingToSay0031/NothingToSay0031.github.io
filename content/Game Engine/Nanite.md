...
#### 探索解决方案：体素化 (Voxelization) 与符号距离场 (SDF)
...
#### 现实约束：必须兼容现有内容创作流程 (Content Creation Workflow)
...
##### Point-Based Rendering (点云渲染)的挑战
...
#### 遮挡剔除的实现：层级Z缓冲 (Hierarchical Z-Buffer)
...
#### 更进一步的遮挡剔除：双通道遮挡剔除（Two-Pass Occlusion Culling）
...
#### 超越深度：解耦可见性与材质（Decoupling Visibility from Materials）
...
####  迈向真正的“虚拟化”：按需加载（On-Demand Streaming）
...
####  核心思想：交替变化的群组边界 (Alternating Group Boundaries)
...
####  构建流程：自下而上的群集简化 (Bottom-up Cluster Simplification)
...
####  DAG 的构建流程：合并与分裂 (Merging and Splitting)
...
####  误差度量的挑战与感知启发 (The Challenge of Error Metrics)
...
####  实现唯一切割点：单调误差函数 (Monotonic Error Function)
...
####  混合 BVH 与 Cluster 剔除以提升 GPU 利用率
...
####  硬件光栅化处理微三角形的困境 (The Dilemma of Hardware Rasterization for Micropolygons)
...
####  软件光栅化：一种更优的解法 (Software Rasterization: A Better Solution)
...
####  软件方案的代价与挑战 (The Costs and Challenges of a Software Approach)
...
####  扫描线光栅化：优化内循环 (Scanline Rasterization: Optimizing the Inner Loop)
...
####  当前解决方案：可见性缓冲区代理体 (Visibility Buffer Imposters)
...
####  从全屏渲染到分块光栅化 (Tiled Rasterization)
...
####  解决方案：解析微分 (Analytic Derivatives)
...
####  虚拟阴影贴图 (Virtual Shadow Maps - VSM) 详解
...
#### 基于“需求页面掩码”的剔除 (Culling based on "Needed Pages Mask")
...
##### 挑战：与虚拟纹理（Virtual Texturing）的根本不同
...
#### 请求的生成与特点 (Request Generation & Characteristics)
...
#### 异步处理流程 (Asynchronous Processing Workflow)
...
#### 内存表示格式 (In-Memory Representation)
...
####  顶点属性编码 (Vertex Attribute Encoding)
