...
#### 新方案：屏幕空间辐射缓存（Screen Space Radiance Caching）
...
#### 屏幕空间辐射缓存（Screen Space Radiance Cache）详解
...
## 自适应探针放置策略 (Adaptive Probe Placement)
...
####  结构化重要性采样 (Structured Importance Sampling - SIS)
...
####  渲染管线集成 (Integration into the Rendering Pipeline)
...
####  技术方案：世界空间辐射缓存 (World Space Radiance Cache - WSRC)
...
###  世界空间辐射缓存（World Space Radiance Cache）的结构与策略
...
##### 最终方案：球谐光照 (Spherical Harmonics, SH)
...
####  核心优势：与光线追踪实现解耦 (Ray Tracing Agnostic)
